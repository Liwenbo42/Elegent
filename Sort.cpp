...
void insert_sotr(int *first, int *last, function<bool(int ,int)> cmp = less<int>()){
    int *ind = first;
...
void sotr(int *first, int *last, function<bool(int ,int)> cmp = less<int>()){
    intro_loop(first, last, cmp);
    insert_sotr(first, last, cmp);
    return;
}
int main(){
    int arr[100], n;
...
void insert_sotr(RandomIter first, RandomIter last, function<bool(int ,int)> cmp = less<int>()){
...
void sotr(RandomIter first, RandomIter last, function<bool(int ,int)> cmp = less<int>()){
    intro_loop(first, last, cmp);
    insert_sotr(first, last, cmp);
    return;
}
